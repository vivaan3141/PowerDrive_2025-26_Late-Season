...
chassis.slew_drive_set(true);  // Enables global slew
 ...
 chassis.pid_drive_set(-1_in, 127,false);
  chassis.pid_wait();
  loader.set(false);  
  intake.move(0);
  intake.move(-50);
  ...
  //Turn 135 left 
  chassis.pid_turn_set(-45_deg, 127, false);
  chassis.pid_wait();
  //Back 15 inches

  chassis.pid_drive_set(14.25_in, 127);
  chassis.pid_wait();
  intake.move(0);
...
intake.move(0);
  ...
  chassis.pid_wait();
   chassis.pid_swing_set(ez::LEFT_SWING, 215, 110); 
  chassis.pid_wait_quick_chain();
  loader.set(true);  
  pros::delay(250);
...
void elimLongGoalLeft() {
  ...
  chassis.pid_swing_set(ez::LEFT_SWING, 0, 120); 
  chassis.pid_wait();

  chassis.pid_drive_set(11_in, 127,false);
  chassis.pid_wait();
  intake.move(100);
  loader.set(true);  
  pros::delay(500);
 ...
 chassis.pid_drive_set(-3_in, 127,false);
  chassis.pid_wait();
  //Turn 135 left 
...
intake.move(0);
  scorer.move(0);
   chassis.pid_drive_set(47_in, 100);
  chassis.pid_wait();
loader.set(true);  
  pros::delay(500);
  chassis.pid_turn_set(180_deg, 127, false);
...
void skills(){
  ...
  //turn to matchloader
  chassis.pid_drive_set(32_in, 63.5);
  chassis.pid_wait();
  loader.set (true);
  ...
  chassis.pid_drive_set(9.5_in, 63.5);
  chassis.pid_wait();
  ...
  pros::delay (500);
  chassis.pid_drive_set(-8.6_in, 50); //og -18 -2/12
  chassis.pid_wait();

  intake.move (127); //scoreTop
    scorer.move (127);

  pros::delay (2500); // scored first match loader
  ...
  //drive to long goal
  chassis.pid_drive_set(-25_in, 63.5); //backing to goal to score match loader 2
  chassis.pid_wait();
  intake.move (127); //scoreTop
  scorer.move (127); //scoreTop
  pros::delay (6000); // scored first match loader
